...
class EgressBroker:
    ...
    Security properties:
      ...
      - DNS answers are validated exactly once
      - the connection is pinned to a validated address
      - TLS still verifies the original hostname
      - redirects are denied
      - proxy environment variables are bypassed
    """
    ...
    def fetch(
        ...
        *,
        ...
        # Connection uses this validated address directly. No second DNS lookup.
        ...
        try:
            connection.request(
                ...
                headers={
                    "Host": hostname,
                    "User-Agent": "BULL-EgressBroker/2",
                    "Accept": "*/*",
                    "Connection": "close",
                },
            )
            ...
            if 300 <= response.status < 400:
                raise EgressDenied("redirects are disabled")
    ...
    def _resolve_public_addresses(
        ...
        for info in infos:
            ...
            if (
                ...
                # Mixed public/private DNS answers also fail closed.
                raise EgressDenied(
                    "destination resolved to non-public address: "
                    f"{address}"
                )
    ...
    *,
    ...
    try:
        ...
        if not response.get("ok"):
            raise EgressDenied(
                response.get("error", "egress request denied")
            )
