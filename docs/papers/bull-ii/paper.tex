...
\author{Luis Minier\\Anharmoniclabs\\\small\href{https://orcid.org/0009-0006-7321-4167}{ORCID: 0009-0006-7321-4167}}
\date{September 23, 2026\\\small Revised evidence edition, revision 2\\Validated runtime: \texttt{ab53f563bcbcaf60820acb318b8212796ce502ff}}
\begin{document}
\maketitle
\begin{abstract}
...
\[
\operatorname{Execute}(r)\Rightarrow
\operatorname{Allowed}(r)\land\operatorname{Bound}(r)\land
\operatorname{Admitted}(r)\land\operatorname{Protected}(r).
\]
...
\begin{figure}[ht]
\centering\includegraphics[width=\linewidth]{figures/data-flow.pdf}
...
\begin{figure}[ht]
\centering\includegraphics[width=\linewidth]{figures/microvm-io.pdf}
\caption{Read-only guest inputs and separate control/audit routes, from the repository drawing. Workload text cannot act as a trusted status frame.}
\end{figure}
...
\bottomrule
\end{tabularx}\caption{An orchestration test is not evidence that an external adapter's effects use production enforcement.}
\end{table}
...
\begin{figure}[ht]
\centering\includegraphics[width=\linewidth]{figures/audit-flow.pdf}
...
\begin{figure}[ht]
\centering\includegraphics[width=0.85\linewidth]{figures/regression-milestones.pdf}
...
The finite TLA+ models cover runtime, session/audit, and approval transitions. The recorded model sizes are 405, 57, and 112 distinct states respectively. All parse/model gates passed. They check the configured finite specifications, not an automatically established refinement from all Python code or every environment to the model.
...
The website now links this revised paper and its companion. The older Paper I PDF remains separately archived. The existing Zenodo record remains the prior deposited edition until a new version is uploaded there; a Git push does not amend that deposit or assign this PDF a new DOI. The previous Paper II's SHA-256 is preserved in \code{evidence-manifest.json} to distinguish the documents.
...
\bibitem{prior} L. Minier, \emph{BULL: Blocking Unauthorized Logic Loopholes}, Zenodo record containing Paper I and the preceding Paper II, September 23, 2026. \url{https://doi.org/10.5281/zenodo.22922278}. Prior Paper II publication snapshot: 437de3f.
